...
export function RosterModal({ team, onClose }: RosterModalProps) {
  return (
    <Modal opened={!!team} onClose={onClose} title={team ? `${team.name} roster` : ''} size="xl">
      {team && (
        <Stack>
          <Group>
            <ThemeIcon color="green"><IconFlag size={18} /></ThemeIcon>
            <Stack gap={0}>
              <Text fw={700}>Bảng {team.group} · {team.code}</Text>
              <Text size="xs" c="dimmed">Current squad · fetched {team.rosterFetchedAt}</Text>
            </Stack>
          </Group>
