...
export function PredictPanel({ activeGroup, predictions, resolver, onActiveGroupChange, onScore, onRoster }: PredictPanelProps) {
  return (
    <>
      <SegmentedControl value={activeGroup} onChange={onActiveGroupChange} data={groupKeys.map((group) => ({ label: `Bảng ${group}`, value: group }))} className="group-switch" />
      <SimpleGrid cols={{ base: 1, md: 2, xl: 3 }} spacing="md" mt="md">
        {groupMatches.filter((match) => match.group === activeGroup).map((match) => (
          <MatchCard key={match.id} match={match} prediction={predictions[match.id]} resolver={resolver} onScore={onScore} onRoster={onRoster} />
        ))}
      </SimpleGrid>
